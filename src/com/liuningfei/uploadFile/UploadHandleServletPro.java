...
/**
 * Servlet implementation class UploadHandleServletPro
 */
@WebServlet("/UploadHandleServletPro")
public class UploadHandleServletPro extends HttpServlet {
...
//    static final String DB_URL = "jdbc:mysql://localhost:3306/RUNOOB?useUnicode=true&amp;characterEncoding=UTF-8"; // 192.168.1.28
    ...
    // 属性
    static HttpServletResponse responseObj;
    
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UploadHandleServletPro() {
        super();
...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		// 属性赋值
		UploadHandleServletPro.responseObj = response;
					 ...
					 for(FileItem item : list){
						 ...
						 if(item.isFormField()){
							...
							//得到文件保存的名称
							 ...
							 //创建一个文件输出流
							 String finalSavePath = realSavePath + System.getProperty("file.separator") + saveFilename; // savePath + File.separator + filename;
							 /*
							 finalZipFilePath = finalSavePath;
							 ...
							 */
							 ...
							 String insertFileUrl = finalSavePath.substring(savePath.length());
							 System.out.println("上传的文件的路径是："+finalSavePath);
							 System.out.println("数据库保存的文件的路径是："+insertFileUrl);
							 
							 // 修改上传到数据库的文件路径
							 if (System.getProperty("file.separator").equals("/")) { // Mac
								 System.out.println("---->>>> 当前系统 Mac");
							 } else if (System.getProperty("file.separator").equals("\\")) { // Windows
								 System.out.println("---->>>> 当前系统 Windows");
								 insertFileUrl = insertFileUrl.replaceAll("\\\\", "\\\\\\\\");
								 System.out.println("修改后 数据库保存的文件的路径是："+insertFileUrl);
						     };
							 
//							 insertFileUrl = insertFileUrl.replaceAll(System.getProperty("file.separator"),System.getProperty("file.separator") + System.getProperty("file.separator"));
							 FileOutputStream out = new FileOutputStream(finalSavePath);
...
//		String dir = savePath + "\\" + dir1 + "\\" + dir2;  //upload\2\3  upload\3\5
		String dir = savePath + System.getProperty("file.separator") + dir1 + System.getProperty("file.separator") + dir2;
	...
	private void insertUploadFileInfoIntoDatabase(String fileName, String fileUrl) {
		...
		try {
            ...
            String sql;
            sql = "insert into uploadFiles(fileName, fileUrl)values(\'"+ fileName + "\',\'" + fileUrl + "\')";
